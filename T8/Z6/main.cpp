...
template <typename TipElemenata>
 ...
 {  
     ...
     while(pocetak!=nullptr) 
     {
         if(pocetak->element>prag)  brojac++;
         pocetak=pocetak->veza;
     }
     return brojac;
 }
...
int main() {
  ...
  int brojac = BrojVecihOd(pocetak, SumaElemenata(pocetak) / BrojElemenata(pocetak));

    UnistiListu(pocetak);

    std::cout<<"\nU slijedu ima "<<brojac<<" brojeva vecih od njihove aritmeticke sredine";
